<script lang="ts">
	...
	// Store
	function updateDateStore(year: number, month: number, day: number) {
		DateStore.update(() => [{year ,month ,day}]);
	}
</script>
...
>
	...
	<Calendar.Months class="w-full">
		{#each months as month}
			<Calendar.Grid class="flex flex-col items-center justify-center">
				<Calendar.GridHead>
							...
							{#each weekDates as date}
								<Calendar.Cell {date}>
									<Calendar.Day
										{date}
										month={month.value}
										on:click={() => updateDateStore(date.year, date.month, date.day)}
									/>
								</Calendar.Cell>
							{/each}
						</Calendar.GridRow>
